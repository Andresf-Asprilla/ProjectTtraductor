from PySide6.QtWidgets import QMainWindow,QApplication,QButtonGroup, QVBoxLayout, QRadioButton,QHBoxLayout, QWidget, QPushButton,QComboBox ,QLabel,QTextEdit,QGridLayout
...
class Traductor_App(QMainWindow):
    ...
    def voz_Qcombox_objetivo(self):
        _, objetivo = self.verificador_fuente_objetivo(self.diccionario_Google)
        self.funcion_voz(self.lista_textEdit[1].toPlainText(),objetivo)
    def voz_Qcombox_fuente(self):
        fuente, _ = self.verificador_fuente_objetivo(self.diccionario_Google)
        self.funcion_voz(self.lista_textEdit[0].toPlainText(),fuente)

    def funcion_voz(self,texto,lang):
        gTTS(text=texto, lang=lang, tld='us').write_to_fp(voice := NamedTemporaryFile())
    ...
    def Traducir_Idioma(self):
        try:
            if self.indicador_conversor=="Google":
                ...
                self.translated = GoogleTranslator(source=fuente, target=Target).translate(self.lista_textEdit[0].toPlainText())
                ...
                self.translated = MyMemoryTranslator(source=fuente, target=Target).translate(self.lista_textEdit[0].toPlainText())
                self.lista_textEdit[1].setPlainText(self.translated)
